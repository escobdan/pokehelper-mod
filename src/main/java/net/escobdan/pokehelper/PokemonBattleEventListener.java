...
public class PokemonBattleEventListener {
    private MinecraftClient storedClient;
    private BattleStartedPostEvent storedBattleEvent;
    private boolean battleStarted = false;
    private boolean firstPokemonSent = false;

    public PokemonBattleEventListener() {
...
//            });
    ...
    private void onBattleStartedPost() {
        ...
        if(storedClient == null) {
            storedClient = MinecraftClient.getInstance();
        }
        PlayerEntity player = storedClient.player;
        String playerUuid = player.getUuid().toString();

        JsonObject dataJson = JsonCreator(storedBattleEvent, playerUuid);
...
//        System.out.println(dataJsonString);
    ...
    private JsonObject JsonCreator(BattleStartedPostEvent event, String playerUuid) {
        JsonObject dataJson = new JsonObject();
        for(BattleActor battleActor : event.getBattle().getActors()) {
            JsonObject actorJson = new JsonObject();
            // Flag to see if actor is player
            actorJson.addProperty("is_player", playerUuid.equals(battleActor.getUuid().toString()));
...
//            dataJson.add(battleActor.getUuid().toString(), actorJson);
            if(playerUuid.equals(battleActor.getUuid().toString())) {
                dataJson.add("player", actorJson);
            }
            else {
                dataJson.add("enemy", actorJson);
